...
const validationSchema = Yup.object().shape({
  ...
  key: Yup.string().when("is_edit", {
    is: false,
    then: (schema) => schema.required("密钥 不能为空"),
  }),
  other: Yup.string(),
  ...
  base_url: Yup.string().when("type", {
    ...
    then: (schema) => schema.required("渠道API地址 不能为空"), // base_url 是必需的
    otherwise: (schema) => schema.notRequired(), // 在其他情况下，base_url 可以是任意字符串
  }),
  allow_streaming: Yup.number().required("流式请求 不能为空"),
  allow_non_streaming: Yup.number().required("流式请求 不能为空"),
...
const EditModal = ({ open, channelId, onCancel, onOk }) => {
  ...
  const submit = async (values, { setErrors, setStatus, setSubmitting }) => {
    ...
    if (values.allow_streaming === 2 && values.allow_non_streaming === 2) {
      setStatus({ success: false });
      showError("流式和非流式不能同时关闭！");
      setErrors({ submit: "流式和非流式不能同时关闭！" });
      return;
    }
        ...
        >
          {({
            ...
            <form noValidate onSubmit={handleSubmit}>
              ...
              <FormControl
                fullWidth
                error={Boolean(touched.allow_streaming && errors.allow_streaming)}
                sx={{ ...theme.typography.otherInput }}
              >
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={values.allow_streaming === 1}
                      onChange={() => {
                        setFieldValue('allow_streaming', values.allow_streaming === 1 ? 2 : 1);
                      }}
                      name="allow_streaming"
                    />
                  }
                  label={inputLabel.allow_streaming}
                />
                {touched.allow_streaming && errors.allow_streaming ? (
                  <FormHelperText error id="helper-text-allow-streaming">
                    {errors.allow_streaming}
                  </FormHelperText>
                ) : null}
              </FormControl>
              ...
              <FormControl
                fullWidth
                error={Boolean(touched.allow_non_streaming && errors.allow_non_streaming)}
                sx={{ ...theme.typography.otherInput }}
              >
                <FormControlLabel
                  control={
                    <Checkbox
                      la
                      checked={values.allow_non_streaming === 1}
                      onChange={() => {
                        setFieldValue('allow_non_streaming', values.allow_non_streaming === 1 ? 2 : 1);
                      }}
                      name="allow_non_streaming"
                    />
                  }
                  label={inputLabel.allow_non_streaming}
                />
                {touched.allow_non_streaming && errors.allow_non_streaming ? (
                  <FormHelperText error id="helper-text-allow-non-streaming">
                    {errors.allow_non_streaming}
                  </FormHelperText>
                ) : null}
              </FormControl>

...
import { IconBrandWechat, IconBrandGithub, IconMail, IconBrandGoogle, IconBrandDiscord } from '@tabler/icons-react';
import Label from 'ui-component/Label';
import { API } from 'utils/api';
import { showError, showSuccess } from 'utils/common';
import { onGitHubOAuthClicked, onGoogleOAuthClicked, onDiscordOAuthClicked } from 'utils/common';
        ...
        <Card sx={{ paddingTop: '20px' }}>
          <Stack spacing={2}>
            <Stack direction="row" alignItems="center" justifyContent="center" spacing={2} sx={{ paddingBottom: '20px' }}>
              ...
              <Label variant="ghost" color={inputs.google_id ? 'primary' : 'default'}>
                <IconBrandGoogle /> {inputs.google_id || '未绑定'}
              </Label>
            ...
            <SubCard title="账号绑定">
              <Grid container spacing={2}>
                ...
                {status.google_oauth && !inputs.google_id && (
                  <Grid xs={12} md={4}>
                    <Button variant="contained" onClick={() => onGoogleOAuthClicked(status.google_client_id, true)}>
                      绑定 Google 账号
                    </Button>
                  </Grid>
                )}
                {status.discord_oauth && !inputs.discord_id && (
                  <Grid xs={12} md={4}>
                    <Button variant="contained" onClick={() => onDiscordOAuthClicked({
                      discord_client_id: status.discord_client_id,
                      discord_check_guild: status.discord_check_guild,
                      discord_check_role: status.discord_check_role
                    }, true)}>
                      绑定 Discord 账号
                    </Button>
                  </Grid>
